...
Deno.test(
  "FileStorage - exists() - file exists",
  withTemp(async (info) => {
    const storage = createFileStorage(info, Deno.cwd());
...
Deno.test("FileStorage - exists() - file doesn't exist", async () => {
  const storage = createFileStorage(baseSingle, Deno.cwd());
  const result = await storage.exists();
  assertEquals(result, false);
});
...
Deno.test(
  "MultiFileStorage - get() - creates directory structure when missing",
  async () => {
    const storage = createFileStorage(baseMulti, Deno.cwd());
    assertThrowsAsync(() => Deno.stat("__test"), Deno.errors.NotFound);
...
Deno.test(
  "MultiFileStorage - set() - creates directory structure when missing",
  async () => {
    const storage = createFileStorage(baseMulti, Deno.cwd());
    assertThrowsAsync(() => Deno.stat("__test"), Deno.errors.NotFound);

    await storage.set(0, new Uint8Array(baseMulti.pieceLength));
    const result = await Deno.stat("__test");
    assertEquals(!!result, true);
...
Deno.test("MultiFileStorage - exists() - file exists", async () => {
  const storage = createFileStorage(baseMulti, Deno.cwd());
  await Deno.mkdir("__test");
  await Deno.writeFile("__test/__test.txt", Uint8Array.from([1, 2, 3, 4]), {
    create: true,
  ...
  assertEquals(await storage.exists(), true);

  await Deno.remove("__test", { recursive: true });
});

Deno.test("MultiFileStorage - exists() - file doesn't exist", async () => {
  const storage = createFileStorage(baseMulti, Deno.cwd());

  assertEquals(await storage.exists(), false);
});
